...
import { TrendingUp, BarChart3, Calendar, ArrowUpRight, ArrowDownRight, Package } from 'lucide-react';
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      setData({
        ...
        orderCount,
        topProducts: [
          { name: 'Standard Surgical Scissors', sales: 45, revenue: 719.55 },
          { name: 'Disposable Face Masks', sales: 38, revenue: 341.62 },
          { name: 'Digital Blood Pressure Monitor', sales: 22, revenue: 990.00 },
          { name: 'Medical Grade Latex Gloves', sales: 18, revenue: 225.00 },
        ],
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
          ...
          <p className="text-2xl font-bold text-gray-900 mt-1">${data.revenue.toLocaleString()}</p>
        </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {data.topProducts.map((product: any, idx: number) => (
                <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-900 font-bold">${product.revenue.toFixed(2)}</td>
                  <td className="px-6 py-4">
