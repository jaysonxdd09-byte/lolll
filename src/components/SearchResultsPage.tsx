          ...
          <div>
            ...
            <h1 className="text-xl sm:text-2xl font-serif text-gray-900">
              {isWishlist ? 'Your Saved Medical Items' : (
                <> {filteredProducts.length} results for <span className="text-gold-600">"{query || category}"</span> </>
              )}
            </h1>
          </div>
                ...
                <div className="space-y-2">
                  {['All', 'Instruments', 'Surgical Wear', 'Diagnostic', 'Wound Care'].map(cat => (
                    <label key={cat} className="flex items-center gap-3 group cursor-pointer">
                      <input type="checkbox" className="w-4 h-4 rounded border-gray-300 text-gold-500 focus:ring-gold-500/20" />
                      <span className="text-xs text-gray-600 group-hover:text-gold-600 transition-colors">{cat}</span>
                    </label>
                  ))}
                </div>
              </div>
              <div className="space-y-4 mb-8">
                ...
                <div className="space-y-2">
                  {['Under $25', '$25 to $50', '$50 to $100', 'Over $100'].map(price => (
                    <button key={price} className="block text-xs text-gray-600 hover:text-gold-600 transition-colors">{price}</button>
                  ))}
                </div>
              </div>
              <div className="space-y-4">
                <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Customer Reviews</h4>
                <div className="space-y-2">
                  {[4, 3, 2, 1].map(stars => (
                    <button key={stars} className="flex items-center gap-1 group">
                      <div className="flex text-gold-400">{[...Array(5)].map((_, i) => (<Star key={i} className={`w-3 h-3 ${i < stars ? 'fill-current' : 'text-gray-200'}`} />))}</div>
                      <span className="text-xs text-gray-500 group-hover:text-gold-600 transition-colors">& Up</span>
                    </button>
                  ))}
                </div>
              </div>
            </div>
            <div className="bg-gold-500/5 rounded-3xl p-6 border border-gold-500/10">
              <h4 className="text-sm font-serif text-gold-900 mb-2">Expert Consultation</h4>
              <p className="text-[10px] text-gold-700 leading-relaxed mb-4">Need help choosing the right instrument?</p>
              <button className="w-full bg-gold-500 text-white py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-gold-600 transition-all shadow-lg shadow-gold-500/20">Chat with Specialist</button>
            </div>
          </div>
        </aside>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {['All', 'Instruments', 'Surgical Wear', 'Diagnostic', 'Wound Care'].map(cat => (
                      <button key={cat} className="px-4 py-2 rounded-xl text-xs font-bold border border-gray-200 text-gray-600 hover:border-gold-400 hover:text-gold-600">{cat}</button>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {['Under $25', '$25-$50', '$50-$100', '$100+'].map(price => (
                      <button key={price} className="px-4 py-2 rounded-xl text-xs font-bold border border-gray-200 text-gray-600 hover:border-gold-400 hover:text-gold-600">{price}</button>
                    ))}
                  </div>
                </div>
              </div>
              <button onClick={() => setShowMobileFilters(false)} className="w-full mt-6 bg-gold-500 text-white py-4 rounded-xl font-bold text-xs uppercase tracking-widest">Apply Filters</button>
            </div>
          </div>
        )}
          ...
          <AnimatePresence mode="popLayout">
            {filteredProducts.length > 0 ? (
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white rounded-3xl border border-gray-100 shadow-sm py-20 sm:py-32 text-center">
                ...
                <button onClick={onBack} className="bg-gold-500 text-white px-8 py-4 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-gold-600 transition-all shadow-lg shadow-gold-500/20">Return to Home</button>
              </motion.div>
            )}
          </AnimatePresence>
        </main>
