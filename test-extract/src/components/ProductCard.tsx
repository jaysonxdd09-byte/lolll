...
const ProductCard: React.FC<ProductCardProps> = ({ product, onAddToCart, isSaved = false, onToggleSave, onBuyNow, onClick }) => {
      ...
      <div className="p-2.5 sm:p-4 flex flex-col flex-1">
        <div className="flex items-center gap-1.5 mb-2">
          {product.brand === 'Test One' && (
            <span className="bg-primary-50 text-primary-600 border border-primary-200 text-[8px] font-extrabold uppercase px-1.5 py-0.5 rounded">Test One</span>
          )}
          {product.code && (
            <span className="text-[9px] font-mono text-gray-500 font-semibold uppercase">{product.code}</span>
          )}
          {product.gst && (
            <span className="text-[9px] text-gray-500 font-bold ml-auto">GST: {product.gst}</span>
          )}
        </div>

        <h3 className="text-[13px] sm:text-[16px] font-bold text-gray-900 mb-2 sm:mb-3 leading-tight group-hover:text-primary-700 transition-colors line-clamp-2 min-h-[2.4rem] sm:min-h-[3rem]">
          {product.name || 'Untitled Product'}
        </h3>

        <div className="mb-2 sm:mb-4">
          <p className="hidden sm:block text-[11px] uppercase tracking-[0.15em] text-gray-500 font-medium">Institutional Pricing</p>
          <p className="sm:hidden text-[9px] uppercase tracking-[0.12em] text-gray-500 font-medium">Pro Price</p>
        </div>

        <div className="flex items-baseline justify-between mb-3 border-t border-gray-50 pt-3">
          <div className="flex flex-col">
            <div className="flex items-baseline gap-1 flex-wrap">
              <span className="text-[15px] sm:text-[18px] font-black text-gray-900 leading-none">₹{price.toLocaleString('en-IN')}</span>
              {mrpVal > price && (
                <span className="text-[10px] sm:text-[11px] text-gray-500 line-through">₹{mrpVal.toLocaleString('en-IN')}</span>
              )}
            </div>
            {discountPct > 0 && (
        ...
        {stock > 0 && (
          <div className="grid grid-cols-2 gap-1.5 mt-auto pt-1">
            <button 
              ...
              className="w-full h-8 rounded-lg bg-primary-600 text-white hover:bg-primary-700 transition-all active:scale-95 text-[8px] sm:text-[9px] font-bold uppercase tracking-wider flex items-center justify-center shadow-sm shadow-primary-500/10"
            >
              Buy Now
            </button>
            <button 
