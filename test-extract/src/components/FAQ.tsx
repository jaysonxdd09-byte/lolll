    ...
    <section id="faq" className="py-24 bg-white overflow-hidden">
      <div className="max-w-4xl mx-auto px-4 sm:px-8">
        <div className="text-center mb-16">
          <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-primary-500 mb-4 block">Knowledge Base</span>
          <h2 className="text-4xl font-serif text-gray-900 mb-6">Frequently Asked <span className="text-primary-600">Questions</span></h2>
            ...
            <div key={index} className="group">
              ...
              >
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${
                  openIndex === index ? 'bg-primary-500 text-white' : 'bg-gray-50 text-gray-400 group-hover:text-primary-500'
                }`}>
                  {item.icon}
                </div>
                <span className="flex-1 text-sm sm:text-base font-bold text-gray-900">{item.question}</span>
                <div className={`transition-transform duration-300 ${openIndex === index ? 'rotate-180' : ''}`}>
                  {openIndex === index ? <Minus className="w-5 h-5 text-primary-600" /> : <Plus className="w-5 h-5 text-gray-300" />}
                </div>
              </button>
        ...
        <div className="mt-16 p-8 rounded-3xl bg-gray-900 text-center relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-64 h-64 bg-primary-500/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 group-hover:scale-110 transition-transform duration-700" />
          <div className="relative z-10">
            ...
            <button className="bg-primary-500 text-white px-8 py-4 rounded-xl text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-primary-600 transition-all shadow-lg shadow-primary-500/20">
