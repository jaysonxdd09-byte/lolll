    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          ...
          className="bg-primary-500 hover:bg-primary-600 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 shadow-lg shadow-primary-500/20 transition-all"
        >
          ...
          blogs.map((blog) => (
            <div key={blog.id} className="border border-gray-200 rounded-2xl p-4 flex gap-4 items-center group">
              ...
              <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => onEdit(blog)} className="p-2.5 text-gray-400 hover:text-primary-600 bg-gray-50 rounded-xl"><Edit2 className="w-4 h-4" /></button>
        ...
        <div className="fixed inset-0 z-[140] bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-2xl bg-white rounded-3xl border border-gray-100 shadow-2xl p-6 sm:p-8 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={onSave} className="space-y-3">
              <input value={form.title || ''} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} placeholder="Blog title" className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm outline-none focus:border-primary-500" />
              <input value={form.author || ''} onChange={(e) => setForm((p) => ({ ...p, author: e.target.value }))} placeholder="Author" className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm outline-none focus:border-primary-500" />
              <textarea value={form.excerpt || ''} onChange={(e) => setForm((p) => ({ ...p, excerpt: e.target.value }))} placeholder="Short excerpt" className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm min-h-[90px] outline-none focus:border-primary-500" />
              <textarea value={form.content || ''} onChange={(e) => setForm((p) => ({ ...p, content: e.target.value }))} placeholder="Full content" className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm min-h-[140px] outline-none focus:border-primary-500" />
              <input value={form.image || ''} onChange={(e) => setForm((p) => ({ ...p, image: e.target.value }))} placeholder="Image URL (optional)" className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm outline-none focus:border-primary-500" />
              ...
              <button type="submit" disabled={saving} className="w-full bg-primary-500 hover:bg-primary-600 disabled:opacity-60 text-white py-3 rounded-xl text-xs font-bold uppercase tracking-widest">
