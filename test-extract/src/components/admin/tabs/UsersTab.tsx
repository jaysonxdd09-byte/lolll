...
export default function UsersTab() {
  ...
  const handleUpdateRole = async (id: string, newRole: string) => {
    await db.collection('users').update(id, { role: newRole });
    fetchUsers();
  };
            ...
            {users.map((u) => (
              <tr key={u.id} className="hover:bg-gray-50/50">
                ...
                <td className="px-6 py-4 text-right">
                  {u.role === 'admin' ? (
                    <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase text-primary-600"><ShieldCheck className="w-3 h-3" /> Full Access</span>
                  ) : u.role === 'staff' ? (
