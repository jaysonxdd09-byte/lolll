...
import { serverDb } from '../../../lib/serverClient';
...
export default function HeroTab() {
  ...
  const fetchSlides = async () => {
    try {
      setLoading(true);
      const data = await serverDb.collection('hero_slides').getFullList({
        sort: 'order_index'
      });
      setSlides(data);
    } catch (err) {
  ...
  const handleSave = async (slideData: any) => {
    try {
      if (slideData.id) {
        await serverDb.collection('hero_slides').update(slideData.id, slideData);
      } else {
        await serverDb.collection('hero_slides').create(slideData);
      }
      fetchSlides();
    } catch (err) {
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this slide?')) {
      try {
        await serverDb.collection('hero_slides').delete(id);
        fetchSlides();
      } catch (err: any) {
          ...
          <div key={slide.id} className="bg-white border border-gray-200 rounded-xl p-4 flex gap-6 items-center hover:shadow-md transition-all group">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-[10px] font-bold uppercase tracking-widest text-primary-500 bg-primary-50 px-2 py-0.5 rounded">{slide.badge}</span>
            ...
            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
              <button 
                ...
                className="p-2.5 text-gray-400 hover:text-primary-600 transition-colors bg-gray-50 hover:bg-primary-50 rounded-xl"
              >
