  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-24 sm:pt-32 pb-20">
      <div className="max-w-[95%] mx-auto px-4 sm:px-8">
        <BackButton onBack={onBack} label="Back to Home" className="mb-8" />

        <div className="mb-12">
          <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-primary-500 block mb-2">Our Insights</span>
          <h1 className="text-3xl sm:text-5xl font-serif text-gray-900">The <span className="text-primary-600">Health Journal</span></h1>
            ...
            {blogs.map((blog) => (
              ...
              >
                ...
                <div className="p-6 sm:p-8">
                  ...
                  <h2 className="text-xl sm:text-2xl font-semibold text-gray-900 leading-tight mb-4 group-hover:text-primary-600 transition-colors line-clamp-2">{blog.title}</h2>
                  <p className="text-gray-500 text-sm leading-relaxed mb-6 line-clamp-3">{blog.excerpt}</p>
                  <div className="flex items-center justify-between pt-6 border-t border-gray-50">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 bg-primary-50 rounded-full flex items-center justify-center"><User className="w-3.5 h-3.5 text-primary-600" /></div>
                      <span className="text-[11px] font-bold text-gray-600">{blog.author || 'Test One Team'}</span>
                    </div>
                    <span className="w-10 h-10 bg-gray-50 rounded-full flex items-center justify-center text-gray-400 group-hover:bg-primary-500 group-hover:text-white transition-all"><ArrowRight className="w-4 h-4" /></span>
                  </div>
                </div>
              </motion.article>
            ))}
