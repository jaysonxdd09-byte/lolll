...
import { ShoppingCart, Zap, ShieldCheck, Truck, RefreshCcw, Star, Plus, Minus, ChevronRight, CheckCircle2, Package, Heart, MessageCircle, MapPin, Loader2 } from 'lucide-react';
import { Product } from '../data/products';
import ProductCard from './ProductCard';
import { checkPincode, DeliveryInfo } from '../lib/pincodeZones';
...
const ProductDetailsPage: React.FC<ProductDetailsPageProps> = ({ 
  ...
  const [pincodeResult, setPincodeResult] = useState<DeliveryInfo | null>(null);
    ...
    <div className="min-h-screen bg-gray-50 pt-20 sm:pt-32 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 sm:mb-8 gap-3">
          <BackButton onBack={onBack} label="Back to Catalog" />
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-5 space-y-6">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3 mb-3">
                <span className="bg-primary-50 text-primary-600 px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-widest">{product?.brand || 'Premium'}</span>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className={`p-3 rounded-xl border flex items-center gap-3 ${outOfStock ? 'bg-red-50 border-red-100' : 'bg-white border-gray-100'}`}>
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${outOfStock ? 'bg-red-100 text-red-500' : 'bg-primary-50 text-primary-500'}`}><Package className="w-4 h-4" /></div>
            ...
            <div className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-100 shadow-sm space-y-3">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs font-bold uppercase tracking-widest text-gray-500">Check Delivery & Pricing</span>
              </div>
              <div className="flex gap-2">
                <input
                  type="text"
                  value={pincode}
                  onChange={(e) => { setPincode(e.target.value.replace(/\D/g, '').slice(0, 6)); setPincodeResult(null); setPincodeError(''); }}
                  placeholder="Enter 6-digit pincode"
                  className="flex-1 px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100 transition-all"
                  maxLength={6}
                ...
                <button
                  ...
                  className="px-5 py-3 bg-gray-900 text-white rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-primary-600 transition-all disabled:opacity-50 flex items-center gap-2"
                >
                  {checkingPincode ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Check'}
                </button>
              </div>
              {pincodeError && (
                <p className="text-red-500 text-xs font-medium">{pincodeError}</p>
              )}
              {pincodeResult && (
                <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 space-y-2">
                  <div className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                    <span className="text-sm font-bold text-emerald-700">Delivery Available!</span>
                  </div>
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div>
                      <span className="text-gray-500 font-medium">Location</span>
                      <p className="text-gray-900 font-bold">{pincodeResult.city ? `${pincodeResult.city}, ` : ''}{pincodeResult.state}</p>
                    </div>
                    <div>
                      <span className="text-gray-500 font-medium">Estimated Delivery</span>
                      <p className="text-gray-900 font-bold">{pincodeResult.estimatedDays}</p>
                    </div>
                    <div>
                      <span className="text-gray-500 font-medium">Delivery Charge</span>
                      <p className={`font-bold ${pincodeResult.deliveryCharge === 0 ? 'text-emerald-600' : 'text-gray-900'}`}>
                        {pincodeResult.deliveryCharge === 0 ? 'FREE' : `\u20B9${pincodeResult.deliveryCharge}`}
                      </p>
                    </div>
                    <div>
                      <span className="text-gray-500 font-medium">Total Price</span>
                      <p className="text-gray-900 font-bold">\u20B9{((product?.price || 0) * quantity + pincodeResult.deliveryCharge).toFixed(2)}</p>
                    </div>
                  </div>
                  {pincodeResult.zone === 'metro' && (
                    <p className="text-[10px] text-emerald-600 font-bold uppercase tracking-widest mt-1">\u2728 Metro zone \u2014 Free express delivery!</p>
                  )}
                </motion.div>
              )}
            </div>
            ...
            <div className="bg-white p-5 sm:p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4 bg-gray-50 p-1.5 rounded-xl border border-gray-100">
                  <button onClick={() => handleQuantityChange(-1)} disabled={outOfStock} className="w-7 h-7 flex items-center justify-center bg-white rounded-lg shadow-sm hover:text-primary-600 transition-colors disabled:opacity-50"><Minus className="w-3 h-3" /></button>
                  <span className="text-sm font-bold text-gray-900 min-w-[20px] text-center">{quantity}</span>
                  <button onClick={() => handleQuantityChange(1)} disabled={outOfStock} className="w-7 h-7 flex items-center justify-center bg-white rounded-lg shadow-sm hover:text-primary-600 transition-colors disabled:opacity-50"><Plus className="w-3 h-3" /></button>
                </div>
              </div>
                ...
                {outOfStock ? (
                  ...
                  <>
                    <div className="flex flex-col sm:flex-row gap-3">
                      <button onClick={() => onAddToCart(product, quantity)} className="flex-1 bg-gray-900 text-white py-4 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-primary-600 transition-all shadow-lg flex items-center justify-center gap-2">
                        <ShoppingCart className="w-4 h-4" /> Add to Cart
                      </button>
                      <button onClick={() => onBuyNow?.(product)} className="flex-1 bg-primary-500 text-white py-4 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-primary-600 transition-all shadow-lg flex items-center justify-center gap-2">
            ...
            <div className="space-y-4">
              <div className="flex gap-6 border-b border-gray-100">
                {(['description', 'specifications', 'shipping'] as const).map((tab) => (
                  <button key={tab} onClick={() => setActiveTab(tab)} className={`pb-3 text-[10px] font-bold uppercase tracking-widest transition-all relative ${activeTab === tab ? 'text-primary-600' : 'text-gray-400 hover:text-gray-600'}`}>
                    {tab}
                    {activeTab === tab && (<motion.div layoutId="activeTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary-500" />)}
                  </button>
                ))}
              </div>
                  ...
                  <div>
                    ...
                    <ul className="space-y-3 mt-4">
                      <li className="flex items-start gap-3">
                        <div className="mt-0.5 w-5 h-5 rounded-full bg-primary-50 border border-primary-200 flex items-center justify-center flex-shrink-0">
        ...
        {relatedProducts.length > 0 && (
          <section className="mt-16 sm:mt-20">
            <div className="flex items-end justify-between mb-8">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-primary-500 mb-2 block">Similar Products</span>
                <h2 className="text-2xl sm:text-3xl font-serif text-gray-900">Related <span className="text-primary-600">Items</span></h2>
              </div>
            </div>
        ...
        {alsoLike.length > 0 && (
          <section className="mt-16 sm:mt-20">
            <div className="flex items-end justify-between mb-8">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-primary-500 mb-2 block">Recommended</span>
                <h2 className="text-2xl sm:text-3xl font-serif text-gray-900">You May <span className="text-primary-600">Also Like</span></h2>
              </div>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-6">
              {alsoLike.map((p) => (
