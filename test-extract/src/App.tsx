...
import DoctorVideoTestimonials from './components/DoctorVideoTestimonials';
...
export default function App() {
  ...
  const [view, setView] = useState<'home' | 'certificates' | 'search' | 'product-details' | 'admin' | 'wishlist' | 'blogs' | 'blog-details' | 'faq' | 'my-orders'>('home');
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          const sData = await db.collection('hero_slides').getFullList({
            sort: 'order_index'
          });
          setHeroSlides(sData);
        } catch (sError) {
    ...
    const initAuth = async () => {
      try {
        ...
        if (db.authStore.isValid && db.authStore.model) {
    ...
    const unsubscribeAuth = db.authStore.onChange(async (token, model) => {
    ...
    return () => {
      unsubscribeAuth();
      db.collection('products').unsubscribe('*').catch(() => {});
      db.collection('hero_slides').unsubscribe('*').catch(() => {});
      db.collection('orders').unsubscribe('*').catch(() => {});
  ...
  const handleViewChange = (newView: 'home' | 'certificates' | 'search' | 'product-details' | 'admin' | 'wishlist' | 'blogs' | 'blog-details' | 'my-orders' | 'faq') => {
      ...
      const options = {
        ...
        handler: async function (response: any) {
          ...
          try {
            ...
            // Create order item
            await db.collection('order_items').create({
              order_id: orderData.id,
              product_id: checkoutItem.id,
              quantity: 1,
              unit_price: checkoutItem.price
            });
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-gray-900 selection:bg-gold-100 selection:text-gold-800">
      ...
      <Navbar 
        ...
        currentView={view === 'certificates' ? 'certificates' : view === 'wishlist' ? 'wishlist' : view === 'blogs' ? 'blogs' : view === 'faq' ? 'faq' : view === 'my-orders' ? 'my-orders' : 'home'}
      ...
      <main>
        {isLoading ? (
          ...
          <AdminDashboard user={user} userRole={userRole} onBack={() => handleViewChange('home')} />
        ) : view === 'home' ? (
          <>
